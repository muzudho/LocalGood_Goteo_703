...
<div class="widget board">
    ...
    <form action="<?php echo $this['form']['action']; ?>" method="post" enctype="multipart/form-data">
        <dl>
            ...
            <?php endforeach; ?>
                <?php if(empty($this['data']->id) || !empty($this['data']->parent_skill_id)) : ?>
            <dt><?php echo Text::_('Parent_Skill');?></dt>
            <dd>
                <select name="parent_skill_id">
                    <option value=""></option>
                    <?php foreach($this['parent_skill'] as $key => $value) : ?>
                    <option value="<?php echo $value->id;?>" <?php echo $this['data']->parent_skill_id == $value->id ?'selected="selected"' :'';?>><?php echo $value->name?></option>
                    <?php endforeach; ?>
                </select>
            </dd>
            <? endif; ?>
